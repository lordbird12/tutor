...
import {
  ...
  Pagination, PositionProduct, StoreType, AssetSize, Supplier, Division
} from './page.types';
...
*/
  ...
  getProducts(page: number = 0,
    ...
    return this._httpClient.get<{ meta: Pagination; data: PositionProduct[] }>(environment.API_URL + 'api/briefs', {
  ...
  getProductsND(page: number = 0,
    ...
    return this._httpClient.get<{ meta: Pagination; data: PositionProduct[] }>(environment.API_URL + 'api/brief-osms', {
      params: searchParams, headers: new HttpHeaders({ Authorization: `Bearer ${token}` })
    }).pipe(
      tap((response) => {
        this._pagination.next({
...
*/
  ...
  create(data: any): Observable<any> {
    return this._httpClient.post(environment.API_URL + '/api/anowment', data,  { headers: this.httpOptionsFormdata.headers }).pipe(
      switchMap((response: any) => {
        // Return a new observable with the response
        return of(response);
      })
  ...
  //   return this._httpClient.get<any>(environment.API_URL + '/api/class-type').pipe(
  //     tap((data) => {
  ...
  getById(id: any): Observable<any[]> {
    return this._httpClient.get<any[]>(environment.API_URL + '/api/anowment/' + id).pipe(
      tap((meterial) => {
        this._materials.next(meterial);
      })
    );
  ...
  update(data: any): Observable<any> {
    return this._httpClient.post(environment.API_URL + '/api/anowment/update_anowment', data, this.httpOptionsFormdata).pipe(
      switchMap((response: any) => {
        // Return a new observable with the response
        return of(response);
      })
  ...
  getPage(dataTablesParameters: any): Observable<DataTablesResponse> {
    return this._httpClient.get(environment.API_URL + '/api/anowment/page?page=1&limit=10&sortBy=priority:desc', dataTablesParameters).pipe(
      switchMap((response: any) => {
        return of(response);
      })
    );
